...
export default function VideoPlayerModal({ movie, isOpen, onClose }: VideoPlayerModalProps) {
  ...
  const videoUrl =
    ...
    (isTV
      ? selectedEpisode
        ? STREAM_SOURCES[currentServer].url(String(movie.id), selectedSeason, selectedEpisode)
        : null
      : STREAM_SOURCES[currentServer].url(String(movie.id)));

  const iframeKey = isTV
    ? `tv-${movie.id}-s${selectedSeason}e${selectedEpisode}-${currentServer}`
    : `movie-${movie.id}-${currentServer}`;
                  ...
                  <a
                    href={
                      isTV && selectedEpisode
                        ? `https://vidsrc.in/download/tv/${movie.id}/${selectedSeason}/${selectedEpisode}`
                        : !isTV
                          ? `https://vidsrc.in/download/movie/${movie.id}`
                          : '#'
                    }
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`flex items-center gap-1 px-2 md:px-2.5 py-1 text-xs font-medium rounded transition-colors ${
                      (isTV && !selectedEpisode) || fetchingId
                        ? 'bg-white/5 text-netflix-gray cursor-not-allowed opacity-40'
                        : 'bg-amber-500/80 text-white hover:bg-amber-500'
                    }`}
                    onClick={(e) => { if ((isTV && !selectedEpisode) || fetchingId) e.preventDefault(); }}
                    title={
                      isTV && !selectedEpisode
