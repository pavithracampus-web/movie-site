...
export default function SeriesPage() {
  ...
  const fetchPage = useCallback(async (pageNum: number) => {
    const res = await fetch(`/api/movies?type=tv-popular&page=${pageNum}`);
  ...
  const handleSeriesClick = useCallback((tvShow: Movie) => {
    ...
    <div className="min-h-screen bg-netflix-black">
      ...
      <div className="pt-20 md:pt-24 px-4 md:px-12 pb-8">
        ...
        {loading ? (
          ...
          <p className="text-netflix-gray text-center py-20">No TV series found.</p>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {series.map((show) => (
              <button
              ...
              >
                  ...
                  <img
                    ...
                    onError={(e) => {
                      ...
                      if (parent && !parent.querySelector('.fallback')) {
                        ...
                        fallback.innerHTML = `<span class="text-2xl text-netflix-gray">${show.title.charAt(0)}</span>`;
                        parent.appendChild(fallback);
                      }
                    }}
                  />
                  <span className="absolute top-2 right-2 bg-netflix-red/90 text-white text-[10px] px-1.5 py-0.5 rounded font-medium uppercase">
                    TV
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 text-xs text-netflix-light">
                  <span className="text-green-400">{formatRating(show.vote_average)}</span>
                  <span>{formatYear(show.release_date)}</span>
                </div>
              </button>
            ))}
          </div>
